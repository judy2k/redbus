...
public class StopDbUpdateHelper {
	...
	private static final Pattern busUpdateRegex = Pattern.compile(StopDbHelper.DATABASE_VERSION + ".dat-([0-9]+).gz");
	...
	private static class AsyncUpdateTask extends AsyncTask<UpdateRequest, Integer, UpdateRequest> {
		...
		private void getUpdate(UpdateRequest ur) throws MalformedURLException {
			ur.updateData = doGetBinaryUrl(new URL("http://redbus.googlecode.com/files/" + StopDbHelper.DATABASE_VERSION + ".dat-" + ur.updateDate + ".gz"));
		}
